...
std::string RunDepthAnythingModel(OH_AI_ModelHandle model, float *&imageData) {
    // 设置模型输入数据
...
static napi_value modelInit(napi_env env, napi_callback_info info) {
    ...
    const std::string modelName = "depth_anything.ms";
    size_t modelSize;
    // 加载模型
...
static napi_value modelInference(napi_env env, napi_callback_info info) {
    ...
    // 处理结果
    std::string res = RunDepthAnythingModel(modelms, inputData);
    napi_value success_ret;
    const char *constc = nullptr; // 初始化const char*类型，并赋值为空
    constc = res.c_str();         // string类型转const char*类型
    napi_create_string_utf8(env, constc, res.size(), &success_ret);
    LOGI("Exit runDemo()");
    return success_ret;
}
...
static napi_module depthAnythingModule = {
    .nm_version = 1,
    .nm_flags = 0,
    .nm_filename = nullptr,
    .nm_register_func = Init,
    .nm_modname = "depthanything",
    .nm_priv = ((void *)0),
    .reserved = {0},
};

extern "C" __attribute__((constructor)) void RegisterDepthAnythingModule(void) {
    napi_module_register(&depthAnythingModule);
}