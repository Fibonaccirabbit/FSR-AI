...
void SSDModelUtil::getDecodeResult(const float *&branchScores, const float *&branchBoxData, SSDInferResult &result) {
    //    std::string result = "";
    ...
    for (int i = 1; i < 81; i++) {
        ...
        in_indexes.resize(1);
        //        LOGI("in_indexes %{public}d", in_indexes.size());
    ...
    LOGI("outBoxNum %{public}d", outBoxNum);
    ...
    for (int i = 0; i < outBoxNum; ++i) {
        ...
        if (outBuff[i][2] > 7.0) {
            ...
            //        LOGI("label_classes i %d, outBuff %d", i, (int) outBuff[i][1]);
            result.objects[i].name = label_classes[static_cast<int>(outBuff[i][1])];
            // label id

            result.objects[i].prop = outBuff[i][2];
            // scores

            result.objects[i].right = outBuff[i][3];
            // xmin

            result.objects[i].top = outBuff[i][4];
            // ymin

            result.objects[i].left = outBuff[i][5];
            // xmax

            result.objects[i].bottom = outBuff[i][6];
            // ymax
